#!/usr/bin/env python3
"""Build the animated briefing card for the SAIR ACC Challenge repository.
...
Every number on this card is real. The pool size, the move set and the
generator cap are the challenge's own; the eighteen move trivialisation of
AK(2) is this repository's search result, verified by its own verifier. Nothing
here claims a standing the competition has not published, because it has not
opened yet.
...
DECL = [
    "AK(2)   < x, y | r1, r2 >",
    "  r1    x x y^-1 y^-1 y^-1",
    "  r2    x y x y^-1 x^-1 y^-1",
]

# The three ordinary moves, exactly as the challenge lists them.
CODES = [("invert", "r^-1"), ("multiply", "r s"), ("conjugate", "c r c^-1")]
...
def andrews_curtis(i):
    t = i / FRAMES
    im = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(im)
    head(im, d, "Andrews–Curtis",
         "Can every balanced presentation of the trivial group be trivialised?")
    pipeline(d, t, [("presentation", "balanced, two generators"),
                    ("AC moves", "invert, multiply, conjugate"),
                    ("(x, y)", "shortest verified path wins")])

    panel(d, [M, 346, W - M, 452])
    d.text((M + 20, 358),
           "the classical hard case, and the trivialisation this repository found",
           font=f(UI, 13), fill=DIM)
    for k, ln in enumerate(DECL[:2]):
        d.text((M + 20, 384 + k * 24), ln, font=f(MONO, 15),
               fill=ROSE if k == 0 else PALE)
    d.text((M + 20, 432), DECL[2] + " ·" * (int(t * 4) % 4),
           font=f(MONO, 15), fill=PALE)

    stats(d, 470, [("10,115", "presentations in the pool"),
                   ("5", "moves, with stabilisation"),
                   ("8", "generators at most"),
                   ("18", "moves to trivialise AK(2)")])
    foot(d, "Andrews–Curtis Conjecture (ACC) Challenge")
    return im
...
def check_glyphs():
    ...
    missing = set()
    for face, size, lines in ((MONO, 15, DECL + [c for pair in CODES for c in pair]),
                              (SEMI, 30, ["Andrews–Curtis"]),
                              (UI, 19, ["Andrews–Curtis Conjecture (ACC) Challenge"])):
        font = f(face, size)
        for line in lines:
            for ch in line:
...
BUILDS = {"andrews-curtis.gif": andrews_curtis}
